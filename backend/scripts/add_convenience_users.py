...
async def add_convenience_users():
    async with async_session_factory() as session:
        org = await session.scalar(select(Organization).where(Organization.slug == "global-core-infrastructure"))
        if not org:
        ...
        for u_data in users_to_ensure:
            ...
            if org:
                ...
                if not membership:
                    ...
                    print(f"Added user {u_data['email']} to org {org.name} as {u_data['role'].value}")
                else:
